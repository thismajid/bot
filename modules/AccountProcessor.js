...
export default class AccountProcessor {
    ...
    static async sendResultsToServer(results) {
        try {
            ...
            // ✅ اصلاح: استفاده صحیح از fs promises
            const logData = JSON.stringify(logEntry) + '\n';

            // بررسی وجود فایل و ایجاد آن در صورت عدم وجود
            try {
                await fs.access(Constants.RESULTS_FILE);
            } catch {
                await fs.writeFile(Constants.RESULTS_FILE, '', 'utf8');
            }

            // خواندن محتوای فعلی و اضافه کردن لاگ جدید
            const existingContent = await fs.readFile(Constants.RESULTS_FILE, 'utf8');
            await fs.writeFile(Constants.RESULTS_FILE, existingContent + logData, 'utf8');
